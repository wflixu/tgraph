...
export class mxStencil extends mxShape {
	...
	parseConstraint(node: Element) {
		...
		return new mxConnectionConstraint(new mxPoint(x, y), perimeter, name as string);
	};
	 ...
	 */
	computeAspect(shape:mxShape, x:number, y:number, w:number, h:number, direction: string) {
		var x0 = x;
		var y0 = y;
		var sx = w / this.w0;
		var sy = h / this.h0;
	 ...
	 * Draws this stencil inside the given bounds.
	 */
	drawNode(canvas: any, shape: any, node: any, aspect: any, disableShadow: any, paint: any) {
